...
async function get_audio() {
	...
	const stream = await navigator.mediaDevices
		...
		.then(stream => {
			...
			drawTimeData({ timeData, ctx, WIDTH, HEIGHT, analyzer });
			drawFrequency({ frequencyData, ctx, WIDTH, HEIGHT, analyzer });
			alert('Microphone is working properly.');
		})
		.catch(err => {
			console.log(err);
			alert("Couldn't get access to microphone. Please manually give access.");
		});
}

function drawTimeData({ timeData, ctx, WIDTH, HEIGHT, analyzer }) {
	...
	ctx.stroke();
	requestAnimationFrame(() => drawTimeData({ timeData, ctx, WIDTH, HEIGHT, analyzer }));
}
function drawFrequency({ frequencyData, ctx, WIDTH, HEIGHT, analyzer }) {
	...
	requestAnimationFrame(() => drawFrequency({ frequencyData, ctx, WIDTH, HEIGHT, analyzer }));
}
